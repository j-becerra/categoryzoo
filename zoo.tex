...
If $F : \mathcal{C}^{op} \to \mathbf{Set}$ , then
...
For finite categories and $F: \C \to \mathsf{vect}$, this becomes $FA \;\cong\; \int_{X \in \mathcal{C}} \mathbf{Hom}_\Bbbk \big(\mathcal{C}(A,X),\, FX\big), $ ie

\begin{align}
\boxed{
FA \;\cong\; \int_{X \in \mathcal{C}} \mathcal{C}(A,X)^* \otimes_\Bbbk FX
}
\end{align}
...
\subsection{Yoneda for Functors Between Finite Abelian Categories}

\textbf{Setup.} Let $k$ be a field, $\mathcal{C},\mathcal{D}$ finite abelian $k$-linear
categories, and $F : \mathcal{C} \to \mathcal{D}$ a $k$-linear functor.



Recall that any finite abelian category can be seen as a left $\mathsf{vect}$-module category. Or alternatively, a finite abelian category is copowered over $\mathsf{vect}$. This means that there is an action functor $\triangleright : \mathsf{vect} \times \C \to \C$ with natural isomorphisms $$ \C (V \triangleright X, Y) \cong   \mathrm{Hom}_\Bbbk (V, \C(X,Y)).$$ 


In this context, the Yoneda lemma reads (see FSS)


\begin{align}
\boxed{
FA \;\cong\; \int_{X \in \mathcal{C}} \mathcal{C}(A,X)^* \triangleright FX
}
\end{align}
...
\begin{align}
\boxed{
FA \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(X,A) \otimes_\Bbbk FX
}
\end{align}
...
\section{Co-Yoneda for Functors Between Finite Abelian Categories}

\textbf{Setup.} Let $k$ be a field and $\mathcal{C}, \mathcal{D}$ finite abelian
$k$-linear categories (i.e.\ $k$-linear, finite length, with finite-dimensional
Hom-spaces, enough projectives, and finitely many simple objects up to isomorphism).
Let $F : \mathcal{C} \to \mathcal{D}$ be a $k$-linear functor (additive and
$k$-linear on Hom-spaces; no exactness assumed).


In this context the coYoneda lemma reads (see FSS)



\begin{align}
\boxed{
FA \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(X,A) \triangleright FX
}
\end{align}


For a vector space $V$ and an object $X$, $V \triangleright X$ denotes the copower or action of vect on $\C$.
...
The condition of the theorem holds for instance if $A$ is a basic algebra, ie if $A \cong P_1 \oplus \cdots \oplus P_r$ with the $P_i$ pairwise non-isomorphic. So

\begin{corollary}
Any quasi-Frobenius, basic algebra is a Frobenius algebra.
\end{corollary}
