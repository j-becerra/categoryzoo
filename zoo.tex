...
\newcommand{\Lexf}{\mathsf{Lex}^\mathsf{f}}
\newcommand{\Ao}{\cA_{\text{\normalfont \bfseries !}}}
...
\textbf{Setup.} Let $\mathcal{C}$ be a locally small category, $A \in \mathcal{C}$, and
$F : \mathcal{C} \to \mathbf{Set}$ a functor. Write $\mathcal{C}(A,-)$ for the covariant
representable functor $\mathrm{Hom}_{\mathcal{C}}(A,-)$.

\bigskip
\textbf{Classical statement.} There is a bijection, natural in both $A$ and $F$,
\begin{align}
\mathrm{Nat}\big(\mathcal{C}(A,-),\, F\big) \;\cong\; FA,
\end{align}
given by $\eta \mapsto \eta_A(\mathrm{id}_A)$, with inverse sending $x \in FA$ to the
natural transformation with components $\eta_X(f) = (Ff)(x)$ for $f \in \mathcal{C}(A,X)$.

\subsection*{Natural transformations as an end}

Recall that for functors $F,G : \mathcal{C} \to \mathcal{D}$, the set of natural
transformations can be computed as an \emph{end}:
\begin{align}
\mathrm{Nat}(F,G) \;\cong\; \int_{X \in \mathcal{C}} \mathcal{D}(FX, GX).
\end{align}
Here the end is the limit of the diagram sending $X \mapsto \mathcal{D}(FX,GX)$ and
$f : X \to Y$ to the two maps
\begin{align}
\mathcal{D}(FX,GX) \xrightarrow{\ (Gf)_* \ } \mathcal{D}(FX,GY) \xleftarrow{\ (Ff)^* \ } \mathcal{D}(FY,GY),
\end{align}
i.e.\ a family $(\varphi_X)_{X}$ lies in the end iff $Gf \circ \varphi_X = \varphi_Y \circ Ff$
for all $f$ --- exactly naturality.

\subsection*{The ``ninja'' Yoneda lemma}

Applying this to $\mathcal{C}(A,-)$ and $F$ gives the end-form of Yoneda:
\begin{align}
FA \;\cong\; \int_{X \in \mathcal{C}} \mathbf{Set}\big(\mathcal{C}(A,X),\, FX\big).
\end{align}

\begin{proof}
By definition,
\begin{align}
\int_{X} \mathbf{Set}(\mathcal{C}(A,X), FX) = \mathrm{Nat}\big(\mathcal{C}(A,-), F\big),
\end{align}
so it suffices to reprove the classical statement, but it is instructive to see how the
\emph{wedge condition itself} forces everything. A wedge is a family
\begin{align}
\varphi_X : \mathcal{C}(A,X) \longrightarrow FX, \qquad X \in \mathcal{C},
\end{align}
such that for every $f : X \to Y$ the square
$$
\begin{tikzcd}
\mathcal{C}(A,X) \arrow[r, "\varphi_X"] \arrow[d, "f \circ -"'] & FX \arrow[d, "Ff"] \\
\mathcal{C}(A,Y) \arrow[r, "\varphi_Y"'] & FY
\end{tikzcd}
$$
commutes. Take $X = A$, $g \in \mathcal{C}(A,X)$ arbitrary, and apply the square to
$f = g : A \to X$ evaluated at $\mathrm{id}_A$:
\begin{align}
\varphi_X(g) \;=\; \varphi_X\big(g \circ \mathrm{id}_A\big) \;=\; (Fg)\big(\varphi_A(\mathrm{id}_A)\big).
\end{align}
So \textbf{every component $\varphi_X$ is completely determined by the single value}
$x := \varphi_A(\mathrm{id}_A) \in FA$, via
\begin{align}
\varphi_X(g) = (Fg)(x), \qquad g \in \mathcal{C}(A,X).
\end{align}
Conversely, given any $x \in FA$, this formula defines a wedge (functoriality of $F$ gives
exactly the required commuting squares). This exhibits a bijection
\begin{align}
\int_{X} \mathbf{Set}(\mathcal{C}(A,X), FX) \;\xrightarrow{\ \cong\ }\; FA,
\qquad \varphi \longmapsto \varphi_A(\mathrm{id}_A),
\end{align}
with the evaluation map $x \mapsto (g \mapsto (Fg)(x))$ as inverse. \hfill $\blacksquare$
\end{proof}

\subsection*{Remarks}

\begin{itemize}
\item This proof is the ``universal element'' argument in disguise: the wedge condition,
applied at $X=A$ with $g = \mathrm{id}_A$, is nothing but the statement that
$\mathcal{C}(A,-)$ is the \textbf{free/representing} functor on the ``generic element''
$\mathrm{id}_A \in \mathcal{C}(A,A)$, and $x = \varphi_A(\mathrm{id}_A)$ is where that
generic element must go.

\item The contravariant version, for $F : \mathcal{C}^{\mathrm{op}} \to \mathbf{Set}$, reads
\begin{align}
FA \;\cong\; \int_{X\in\mathcal{C}} \mathbf{Set}\big(\mathcal{C}(X,A), FX\big).
\end{align}

\item The end formula is the one that generalizes cleanly to \textbf{enriched} category
theory: for a $\mathcal{V}$-category $\mathcal{C}$ and $F : \mathcal{C} \to \mathcal{V}$,
\begin{align}
FA \;\cong\; \int_{X \in \mathcal{C}} \big[\mathcal{C}(A,X), FX\big],
\end{align}
where $[-,-]$ is the internal hom of $\mathcal{V}$ --- this is the version used e.g.\ for
$\mathrm{Vect}$-enriched or $\mathrm{Cat}$-enriched Yoneda, and it is the natural language
for the \textbf{coend/density formula}
\begin{align}
F \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(X,-) \cdot FX
\end{align}
(every functor is a colimit of representables), which is Yoneda's \emph{co}end dual and
the starting point for Kan extension formulas via the \textbf{Fubini/ninja Yoneda}
manipulations (co-Yoneda reduction).
\end{itemize}
...
\subsection{Bimodules and the hom}



\textbf{Setup.} Let $k$ be a field and $A,B$ finite-dimensional $k$-algebras. Let
$M,M'$ be left $A$-modules and $N,N'$ right $B$-modules, all finite-dimensional over $k$.
As before, $M\otimes_k N$ and $M'\otimes_k N'$ carry natural $A$-$B$-bimodule structures
\begin{align}
a\cdot(m\otimes n)\cdot b = (a\cdot m)\otimes(n\cdot b).
\end{align}

For an $A$-$B$-bimodule $P$, let $\mathrm{Hom}_B(N,P)$ denote the space of right
$B$-module maps $N \to P$ (forgetting the $A$-action on $P$). This carries a left
$A$-module structure via
\begin{align}
(a\cdot\varphi)(n) := a\cdot\varphi(n), \qquad \varphi \in \mathrm{Hom}_B(N,P),\ a\in A,\ n\in N,
\end{align}
which is well-defined precisely because the $A$- and $B$-actions on $P$ commute:
\begin{align}
(a\cdot\varphi)(nb) = a\cdot\varphi(nb) = a\cdot\big(\varphi(n)\cdot b\big)
= \big(a\cdot\varphi(n)\big)\cdot b = (a\cdot\varphi)(n)\cdot b,
\end{align}
so $a\cdot\varphi$ is again $B$-linear.

\bigskip
\textbf{Statement.} There is an isomorphism, natural in $M$, $M'$, $N$, $N'$,
\begin{align}
\boxed{
\mathrm{Hom}_{A\text{-}\mathrm{mod}\text{-}B}\big(M\otimes_k N,\; M'\otimes_k N'\big)
\;\cong\;
\mathrm{Hom}_A\big(M,\; \mathrm{Hom}_B(N,\, M'\otimes_k N')\big).}
\end{align}

\subsection*{Proof}

\begin{proof}
Start from the classical tensor--hom adjunction over $k$,
\begin{align}
\Xi \;:\; \mathrm{Hom}_k(M\otimes_k N,\; M'\otimes_k N')
\;\xrightarrow{\ \cong\ }\;
\mathrm{Hom}_k\big(M,\, \mathrm{Hom}_k(N, M'\otimes_k N')\big),
\qquad
\Xi(\theta)(m)(n) = \theta(m\otimes n).
\end{align}
It suffices to show $\Xi$ restricts to a bijection between the two subspaces of
bimodule/module-compatible maps.

\smallskip
$\theta$ is a morphism of $A$-$B$-bimodules if and only if, for all $a\in A$, $b\in B$,
$m \in M$, $n \in N$,
\begin{align}
\theta\big((a\cdot m)\otimes(n\cdot b)\big) \;=\; a\cdot\theta(m\otimes n)\cdot b.
\end{align}
Writing $\hat\theta := \Xi(\theta)$, i.e.\ $\hat\theta(m)(n) = \theta(m\otimes n)$, this
condition reads
\begin{align}
\hat\theta(a\cdot m)(n\cdot b) \;=\; a\cdot \hat\theta(m)(n)\cdot b
\qquad \text{for all } a,b,m,n. \tag{$\ast$}
\end{align}

\textbf{Setting $a = 1$ in $(\ast)$:} $\hat\theta(m)(n\cdot b) = \hat\theta(m)(n)\cdot b$
for all $m,n,b$, i.e.\ $\hat\theta(m) \in \mathrm{Hom}_B(N, M'\otimes_k N')$ for every
$m \in M$.

\textbf{Setting $b = 1$ in $(\ast)$, given the above:} $\hat\theta(a\cdot m)(n) =
a\cdot\hat\theta(m)(n)$ for all $m,n$, i.e.\
$\hat\theta(a\cdot m) = a\cdot\hat\theta(m)$ in $\mathrm{Hom}_B(N,M'\otimes_k N')$,
using the $A$-module structure defined above. This says exactly that
$\hat\theta : M \to \mathrm{Hom}_B(N, M'\otimes_k N')$ is $A$-linear.

Conversely, if $\hat\theta(m)$ is $B$-linear for every $m$ and $\hat\theta$ itself is
$A$-linear into $\mathrm{Hom}_B(N,M'\otimes_kN')$, then combining the two displayed
identities recovers $(\ast)$ for all $a,b$.

\smallskip
Hence $\theta$ is a bimodule map if and only if $\hat\theta$ lies in
$\mathrm{Hom}_A\big(M, \mathrm{Hom}_B(N, M'\otimes_k N')\big)$, so $\Xi$ restricts to
the desired isomorphism
\begin{align}
\mathrm{Hom}_{A\text{-}\mathrm{mod}\text{-}B}(M\otimes_kN,\, M'\otimes_kN')
\;\xrightarrow{\ \cong\ }\;
\mathrm{Hom}_A\big(M,\, \mathrm{Hom}_B(N,\, M'\otimes_kN')\big),
\qquad \theta \longmapsto \hat\theta. \tag*{$\blacksquare$}
\end{align}
\end{proof}
...
\begin{itemize}
\item This is a genuine adjunction: the functor $(-)\otimes_k N : A\text{-}\mathrm{mod}
\to A\text{-}\mathrm{mod}\text{-}B$ is left adjoint to
$\mathrm{Hom}_B(N,-) : A\text{-}\mathrm{mod}\text{-}B \to A\text{-}\mathrm{mod}$, with unit
and counit induced from the classical tensor--hom adjunction, restricted along the
fully-faithful inclusion of bimodule maps described above. No finite-dimensionality is
needed for this statement (unlike the pure tensor-factorization formula
$\mathrm{Hom}_A(M,M')\otimes_k\mathrm{Hom}_B(N,N')$ from before, which does use it).

\item If $N$ is finite-dimensional, evaluation/coevaluation gives
$\mathrm{Hom}_B(N,P) \cong (N^\vee \otimes_k P)^B$ (right-$B$-invariants for the
diagonal action, using the induced left $B$-action on $N^\vee$), so the formula can be
rewritten as
\begin{align}
\mathrm{Hom}_{A\text{-}\mathrm{mod}\text{-}B}(M\otimes_kN,\,M'\otimes_kN')
\;\cong\;
\mathrm{Hom}_A\big(M,\; (N^\vee\otimes_k M'\otimes_k N')^B\big),
\end{align}
the same evaluation-morphism circle of ideas that appears in the canonical coend
\begin{align}
\mathbb{F} \;=\; \int^{X} X^\vee \otimes X
\end{align}
for a finite tensor category: the coend $\mathbb{F}$ is built from exactly this kind of
evaluation pairing between an object and its dual, and the adjunction above is its
bimodule-level shadow.
\item If $A,B$ are algebras in a finite tensor category $\mathcal{A}$, then the isomorphism reads 
\begin{align}
\boxed{
\mathrm{Hom}_{A\text{-}\mathrm{mod}\text{-}B}\big(M\otimes N,\; M'\otimes N'\big)
\;\cong\;
\mathrm{Hom}_A\big(M,\; \underline{\mathrm{Hom}}_B(N,\, M'\otimes N')\big).}
\end{align}
\end{itemize}



\subsection{coYoneda lemma}



\textbf{Setup.} Let $\mathcal{C}$ be a locally small category and $F : \mathcal{C} \to \mathbf{Set}$
a functor. Write $\mathcal{C}(X,-)$ for the covariant representable functor at $X \in \mathcal{C}$.
The co-Yoneda lemma expresses $F$ itself as a colimit of representables --- it is the
formal statement that ``every functor is built out of its representable pieces.''

\bigskip
\textbf{Statement.} There is an isomorphism, natural in $F$,
\begin{align}
F \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(X,-) \cdot FX,
\end{align}
where $\int^{X}$ denotes the \emph{coend} and $\mathcal{C}(X,-) \cdot FX$ is the copower
(a coproduct of $|FX|$ copies of the functor $\mathcal{C}(X,-)$):
\begin{align}
\mathcal{C}(X,-) \cdot FX \;=\; \coprod_{x \in FX} \mathcal{C}(X,-).
\end{align}
If $F : \mathcal{C}^{op} \to \mathbf{Set}$ then the formula reads
\begin{align}
F \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(-,X) \cdot FX,
\end{align}


Evaluated at an object $A \in \mathcal{C}$, this reads
\begin{align}
FA \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(X,A) \cdot FX.
\end{align}

If the categories are $\Bbbk$-linear, then this reads
\begin{align}
FA \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(X,A) \otimes FX
\end{align}
or in the contravariant case
\begin{align}
FA \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(A,X) \otimes FX.
\end{align}
And in particular for $F=\mathcal{C}(-,B)^*$ we get
\begin{align}
\mathcal{C}(A,B)^* \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(X,A) \otimes \mathcal{C}(X,B)^*.
\end{align}
or similarly taking the contravariant version
\begin{align}
\boxed{
\mathcal{C}(A,B)^* \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(B,X) \otimes \mathcal{C}(A,X)^*.}
\end{align}

\paragraph{Coends as colimits}

Dually to the end description of naturality, the coend
$\int^{X} \mathcal{C}(X,A) \times FX$
is the \emph{coequalizer}
$$
\begin{tikzcd}[column sep=large]
\displaystyle\coprod_{f : X \to Y} \mathcal{C}(Y,A) \times FX
\arrow[r, shift left, "f^* \times \mathrm{id}"]
\arrow[r, shift right, "\mathrm{id} \times Ff"']
&
\displaystyle\coprod_{X \in \mathcal{C}} \mathcal{C}(X,A) \times FX
\arrow[r]
& \int^{X} \mathcal{C}(X,A)\times FX
\end{tikzcd}
$$
That is, elements are pairs $(g,x)$ with $g : X \to A$, $x \in FX$, modulo the relation
\begin{align}
(g \circ f,\, x) \;\sim\; (g,\, (Ff)(x)), \qquad \text{for } f : X \to Y,\ g : Y \to A,\ x \in FX.
\end{align}
We write $[g,x]$ for the equivalence class of $(g,x)$.
...
Define a map
\begin{align}
\Phi : \int^{X} \mathcal{C}(X,A) \times FX \longrightarrow FA, \qquad \Phi[g,x] = (Fg)(x).
\end{align}
This is well-defined on equivalence classes: for $f : X \to Y$,
\begin{align}
\Phi[g\circ f, x] = \big(F(g\circ f)\big)(x) = (Fg)\big((Ff)(x)\big) = \Phi[g, (Ff)(x)],
\end{align}
using functoriality of $F$, which is exactly the coequalizer relation.

Conversely, define
\begin{align}
\Psi : FA \longrightarrow \int^{X} \mathcal{C}(X,A) \times FX, \qquad
\Psi(a) = [\mathrm{id}_A,\, a].
\end{align}

\textbf{$\Phi \circ \Psi = \mathrm{id}$:} $\Phi\Psi(a) = \Phi[\mathrm{id}_A,a] = (F\mathrm{id}_A)(a) = a$.

\textbf{$\Psi \circ \Phi = \mathrm{id}$:} for any representative $(g,x)$ with $g : X \to A$,
\begin{align}
\Psi\Phi[g,x] = \Psi\big((Fg)(x)\big) = [\mathrm{id}_A, (Fg)(x)] = [\mathrm{id}_A \circ g,\, x] = [g,x],
\end{align}
using the coequalizer relation with $f = g$. Hence $\Phi$ and $\Psi$ are mutually inverse
bijections, natural in $A$ and $F$. \hfill $\blacksquare$
\end{proof}

\paragraph{Remarks}

\begin{itemize}
\item Every element of $\int^{X} \mathcal{C}(X,A) \times FX$ can be represented (non-uniquely)
as $[g,x]$ with $g : X \to A$, $x \in FX$; the coend relation says exactly that ``pushing
$f$ along $g$'' and ``pushing $f$ along $F$'' give the same class. The isomorphism above
shows the \emph{canonical} representative of $a \in FA$ is $[\mathrm{id}_A, a]$.

\item This is the coend/co-Yoneda dual of the end form of Yoneda,
\begin{align}
FA \;\cong\; \int_{X \in \mathcal{C}} \mathbf{Set}\big(\mathcal{C}(A,X), FX\big),
\end{align}
and the two are related by the general \textbf{Yoneda--density} philosophy: $F$ is
simultaneously a limit of the diagram of hom-sets \emph{into} it (end form) and a colimit
of representables glued along $F$'s values (coend form).

\item The enriched version, for $F : \mathcal{C} \to \mathcal{V}$ with $\mathcal{C}$ a
$\mathcal{V}$-category and $\mathcal{V}$ cocomplete closed symmetric monoidal, reads
\begin{align}
F \;\cong\; \int^{X \in \mathcal{C}} \mathcal{C}(X,-) \otimes FX,
\end{align}
with $\otimes$ the tensoring of $\mathcal{V}$-objects over $\mathcal{V}$-functors. This is
the formula underlying the \textbf{density theorem} (every presheaf is a colimit of
representables) and is the standard tool for \textbf{Fubini/ninja Yoneda} manipulations,
e.g.\ rewriting coends of Hom functors such as
\begin{align}
\int^{X \in \mathcal{C}} \mathcal{C}(X,A) \otimes G(X) \;\cong\; G(A)
\end{align}
for any $G : \mathcal{C} \to \mathcal{V}$ --- the ``co-Yoneda reduction'' used constantly
when simplifying coend expressions (e.g.\ in the canonical coend algebra
$F = \int^{X} X^\vee \otimes X$ for a finite tensor category).
\end{itemize}
...
In other words, a right-exact functor from a category with enough projectives is determined by its restriction to projective objects.
...
An object \(P\) in an abelian category \(\mathcal{A}\) is \emph{projective} if for every epimorphism \(p: X \to Y\) and every morphism \(f: P \to Y\), there is a morphism \(f': P \to X\) that lifts \(p\), i.e., \(f = p \circ f'\):
\[
\begin{tikzcd}
& X \arrow[d, "p"'] \\
P \arrow[r, "f"] \arrow[ur, dashed, "\exists f'"] & Y
\end{tikzcd}
\]

An object \(I\) in an abelian category \(\mathcal{A}\) is \emph{injective} if for every monomorphism \(i: Y \to X\) and every morphism \(f: Y \to I\), there is a morphism \(f': X \to I\) that extends \(i\), i.e., \(f = f' \circ i\):
\[
\begin{tikzcd}
Y \arrow[r, "f"] \arrow[d, "i"'] & I \\
X \arrow[ur, dashed, swap,"\exists f'"]
\end{tikzcd}
\]

One can show, as for the category of modules, that an object \(P\) is projective if and only if the functor
\[
\operatorname{Hom}_{\mathcal{A}}(P,-) : \mathcal{A} \to \mathsf{Ab}
\]
is exact. Similarly, an object \(I\) is injective if and only if the functor
\[
\operatorname{Hom}_{\mathcal{A}}(-,I) : \mathcal{A}^{\mathrm{op}} \to \mathsf{Ab}
\]
is exact.



Another characterisation: an object $P$ is projective if and only if every short exact sequence $$  0 \to X \to Y \to P \to 0 $$ is split. Similarly, an object $I$ is injective if and only if every short exact sequence $$  0 \to I \to X \to Y \to 0 $$ is split. 
...
\subsection{Direct sums and summands of projective and injective objects}


\textbf{Proposition:} Let $X:= X_1 \oplus X_2$. Then we have that $X$ is projective (resp. injective) if and only if so are $X_1$ and $X_2$. Ie, direct sums and direct summands of projective and injective objects are of the same type.


\begin{proof}
We prove the statement for injective objects; the projective case is dual.

\medskip

\noindent
\emph{($\Rightarrow$)} Suppose $X = X_1 \oplus X_2$ is injective. We show that $X_1$ and $X_2$ are injective. We treat $X_1$; the argument for $X_2$ is identical.

Let $j: A \hookrightarrow B$ be a monomorphism and $f: A \to X_1$ a morphism. Consider the composition
\[
i_1 \circ f: A \to X,
\]
where $i_1: X_1 \to X_1 \oplus X_2$ is the canonical inclusion.

Since $X$ is injective, there exists $g: B \to X$ such that
\[
g \circ j = i_1 \circ f.
\]
Let $p_1: X \to X_1$ be the projection. Define
\[
g_1 := p_1 \circ g: B \to X_1.
\]
Then
\[
g_1 \circ j = p_1 \circ g \circ j = p_1 \circ i_1 \circ f = \mathrm{id}_{X_1} \circ f = f.
\]
Thus $f$ extends, and $X_1$ is injective.

\medskip

\emph{($\Leftarrow$)} Suppose $X_1$ and $X_2$ are injective. Let
\[
0 \to X_1 \oplus X_2 \xrightarrow{i} M \to N \to 0
\]
be a short exact sequence. Let $p_k: X_1 \oplus X_2 \to X_k$ be the projections, and set
\[
i_k := p_k \circ i: X_1 \oplus X_2 \to X_k.
\]
Then each $i_k$ restricts to a monomorphism $X_k \hookrightarrow M$. Since $X_k$ is injective, there exists a retraction
\[
r_k: M \to X_k
\]
such that $r_k \circ i = p_k$.

Define
\[
r := (r_1, r_2): M \to X_1 \oplus X_2.
\]
Then
\[
r \circ i = (r_1 \circ i, r_2 \circ i) = (p_1, p_2) = \mathrm{id}_{X_1 \oplus X_2},
\]
so the sequence splits. Hence $X_1 \oplus X_2$ is injective.

\medskip


The proof for projective objects is dual: one replaces monomorphisms by epimorphisms and uses the lifting property for projectives.
\end{proof}



\subsection{Projective cover and injective hull}


Let $\C$ be a finite category and  $X \in \mathcal{C}$.

\smallskip
\noindent \textbf{(Projective cover.)} A \emph{projective cover} of $X$ is a projective object $P(X) \in \mathcal{C}$ together with an epimorphism
\[
p \colon P(X) \twoheadrightarrow X
\]
such that for every epimorphism $g \colon P \twoheadrightarrow X$ from a projective object $P$, there exists an epimorphism $h \colon P \twoheadrightarrow P(X)$ such that
\[
p h = g.
\]
...
\smallskip
\noindent \textbf{(Injective hull.)} An \emph{injective hull} of $X$ is an injective object $I(X) \in \mathcal{C}$ together with a monomorphism
\[
\iota \colon X \hookrightarrow I(X)
\]
such that for every monomorphism $f \colon X \hookrightarrow I$ into an injective object $I$, there exists a monomorphism $k \colon I(X) \hookrightarrow I$ such that
\[
k \iota = f.
\]

If an injective hull $I(X)$ of $X$ exists, then it is unique up to a (non-unique) isomorphism.




\subsection{Socle and top}


If $\C$ is a finite category and $X \in \C$, the \textit{socle} if $X$ is the maximal semisimple subobject of $X$, ie, the direct sum of all its simple subobjects,

$$\mathrm{soc}(X)= \text{sum of all simple subobjects of }X.$$ Similarly, the \textit{top} of $X$ is the maximal semisimple quotient,

$$\mathrm{top}(X)= X/\mathrm{rad}(X). $$

Here $\mathrm{rad}(X)$ is the intersection of all maximal subobjects of $X$ ($M \subset X$ maximal if $X/M$ is simple). 



\begin{proposition}
There are bijections
$$\begin{tikzcd}[column sep=3em, row sep=1em]
\left\{ \parbox[c]{8em}{\centering     {\small  \textnormal{simple objects of $\C$ }}} \right\}  \ar[-,double line with arrow={-,-}]{r}  &     \left\{ \parbox[c]{8em}{\centering     {\small  \textnormal{ indecomposable projective objects of $\C$}}} \right\} \\
S \rar & P(S)\\
 \mathrm{top}(P) & P \lar
\end{tikzcd}$$
and 
$$\begin{tikzcd}[column sep=3em, row sep=1em]
\left\{ \parbox[c]{8em}{\centering     {\small  \textnormal{simple objects of $\C$ }}} \right\}  \ar[-,double line with arrow={-,-}]{r}  &     \left\{ \parbox[c]{8em}{\centering     {\small  \textnormal{ indecomposable injective objects of $\C$}}} \right\} \\
S \rar & I(S)\\
 \mathrm{soc}(P) & I \lar
\end{tikzcd}$$
where $P(S)$ and $I(S)$ are the projective cover and injective hull of $S$.
\end{proposition}
...
For instance: in a finite tensor category, the socle of an indecomposable projective object is simple. For  if $P$ is indecomposable projective, in a finite tensor category it is also injective and indecomposable, ie there is a simple object (unique!) such that $P$ is its injective hull. So an indecomposable projective has a unique simple subobject , ie the socle is that precise object ie simple.
...
\subsection{Nakayama functors: classical setting}


Let $A$ be a fin dim algebra and write $\Hom_A(-,-)$ for the hom of the category $A$-mod of fin dim $A$-modules.

Both $A$ and $A^*$ can be viewed as a left/right/bi module with the regular  action, ie $a \cdot b := ab$ for the left one; and the co-regular left/right/bimod action which is the dual of the regular one: for the left one this is multiplication by $a$ in $A^*$ is the map $(a \cdot -)^*$, ie $(a \cdot \omega)(b)= \omega (ab)$.


The left and right Nakayama functors of $A$-mod are the functors $A-mod \to A-mod$ given by $$\mathsf{N}^r_A :=\Hom_A(-,A)^* \qquad , \qquad \mathsf{N}^l_A:= \Hom_{A^{op}}(-^*,A).$$

This requires some explanation: first of all if $V$ is a left module then $\Hom_A(V,A)$ is a right module with $(f \cdot a)(v)= f(a\cdot v)$. On the other hand if $V$ is a right $A$-module then $V^*$ is a left $A$-module with $(a \cdot \omega)(v)=\omega (v \cdot a)$. Also recall that $A^{op}$-mod$ \cong $mod-$A$.

One can show, see [Ivanov prop 3.1] that 

$$ \mathsf{N}^r_A \cong A^* \otimes_A - \qquad , \qquad \mathsf{N}^l_A \cong \Hom_A(A^*,-).$$

A word on this: recall that given a right $A$-module $M$ and a left $A$-module $N$, we have $M \otimes_A N := M \otimes_\Bbbk N / (ma\otimes n - m \otimes an)$. In particular if $M$ is an $(B,A)$-bimodule then $M \otimes_A N$ is a left $B$-module. 


Returning to Nakayama functors, if we view $A^*$ as a $A$-bimodule with the left and right co-regular action, we get that $A^* \otimes_A N$ is a left module whenever so is $N$ . In particular it is very easy to see that this functor is right exact ie preserves cokernels. A similar note goes for the left exact cousin.



\begin{theorem}[{[Iv, Sect. 3]} ] 
We have
\begin{enumerate}


\item The right Nakayama functor takes projective objects to injective objects.
\item The left Nakayama functor takes injective objects to projective objects.
\item Restriction  induces an equivalence of categories $$ \mathsf{N}^r_A : \mathsf{Proj}(A\text{-}\mathsf{mod}) \overset{\simeq}{\longrightarrow}   \mathsf{Inj}(A\text{-}\mathsf{mod}) : \mathsf{N}^l_A .$$
\end{enumerate}
\end{theorem}
\begin{proof}[Proof of (1)]
Let $A$ be a finite-dimensional algebra over a field $k$, and let 
\[
\mathsf{N}^r_A = - \otimes_A A^*
\]
be the right Nakayama functor, where $(-)^* = \operatorname{Hom}_k(-,k)$.

Let $P$ be a projective $A$-module. Since $P$ is projective, it is a direct summand of a finite free module, i.e. there exists $n \in \mathbb{N}$ such that
\[
P \oplus K A^{\oplus n}.
\]
Since the functor $- \otimes_A A^*$ is additive and preserves direct summands, and direct sums and direct summands of injective objects are injective,  it suffices to prove that
\[
A \otimes_A A^* \cong A^*
\]
is injective as an $A$-module.

We now show that $A^*$ is injective. For any right $A$-module $M$, there is a natural isomorphism
\[
\operatorname{Hom}_A(M, A^*) \cong \operatorname{Hom}_k(M,k) = M^*,
\]
given by sending a morphism $f: M \to A^*$ to the linear functional
\[
m \mapsto f(m)(1).
\]
One checks that this construction is functorial and defines a natural isomorphism.

Since $k$ is a field, the functor $(-)^* = \operatorname{Hom}_k(-,k)$ is exact. Hence the functor
\[
\operatorname{Hom}_A(-, A^*)
\]
is exact, and therefore $A^*$ is an injective right $A$-module.

It follows that $A \otimes_A A^*$ is injective. Since injective modules are closed under finite direct sums and direct summands, we conclude that
\[
P \otimes_A A^*
\]
is injective for every projective module $P$.

This proves that the right Nakayama functor takes projective objects to injective objects.
\end{proof}




\subsection{The Nakayama permutation}


A natural question is under which conditions the right and left Nakayama functors  are equivalences.



\begin{lemma}
The following properties of a finite-dimensional $\Bbbk$-algebra $A$ are equivalent:
\begin{enumerate}
    \item[(i)] The right and left Nakayama functors are quasi-inverse of each other.
    \item[(ii)] $A$ is injective as a left $A$-module,
    \item[(iii)] $A$ is injective as a right $A$-module,
    \item[(iv)] The class of (left or right) projective modules coincides with the class of (left or right) injective modules.
\end{enumerate}
\end{lemma}



An algebra with any (and thus all) of these properties is called \emph{self-injective} or \textit{quasi-Frobenius}.


 For self-injective $A$ the right and left Nakayama functors   are in particular exact and there are natural isomorphisms
\begin{equation}
    \mathsf{N}_A^{\mathrm r} \cong {}_A\langle {}_A\langle A^*, A\rangle, -\rangle
    \qquad \text{and} \qquad
    \mathsf{N}_A^{\mathrm l} \cong {}_A\langle A^*, A\rangle \otimes_A -,
    \label{eq:3.75}
\end{equation}
(here ${}_A\langle A^*, A\rangle, -\rangle$ is seen as a bimodule).



So if $A$ is self-injective aka quasi-Frobenius, injective and projective objects are the same. So the bijections between simple objects and indecomposable inj/proj can be composed:


$$\begin{tikzcd}[column sep=3em, row sep=1em]
\left\{ \parbox[c]{8em}{\centering     {\small  \textnormal{simple objects of $\C$ }}} \right\}  \ar[-,double line with arrow={-,-}]{r}  &     \left\{ \parbox[c]{8em}{\centering     {\small  \textnormal{ indecomposable projective  objects =  indecomposable 	injective  objects of $\C$}}} \right\}  \ar[-,double line with arrow={-,-}]{r}  &   \left\{ \parbox[c]{8em}{\centering     {\small  \textnormal{simple objects of $\C$ }}} \right\}  \\
S \rar & P(S) \rar & \mathrm{soc}(P(S))
\end{tikzcd}$$

This definies a permutation on the set of simples of $\C$ called the \textit{Nakayama permutation} and denoted $\nu$.  Sometimes it is useful to write $\mathcal{S}(A)$ for the set of isoclasses of simple $A$-modules and $$\nu: \mathcal{S}(A) \to \mathcal{S}(A) \qquad  , \qquad S_i \mapsto S_{\nu (i)}$$, ie viewing $\nu$ as permuting the indices of the simple objects. In this language if $P_i =P(S_i)$ and $I_j =I(S_j)$

$$ \mathrm{top}(P_i)= S_i    \qquad  , \qquad   \mathrm{soc}(P_i)= S_{\nu (i)}   $$ and 
$$ \mathrm{top}(I_j)= S_{\nu^{-1} (j)}    \qquad  , \qquad   \mathrm{soc}(I_j)= S_j  . $$ 




It interacts nicely with the right Nakayama functor :


\begin{proposition}\label{prop:Naka}
Let $A$ be a self-injective fin dim algebra. Then
\begin{enumerate}
\item $\mathsf{N}^r(P(S)) \cong I(S)$,
\item $\mathsf{N}^r(S)\cong \nu^{-1}(S),$ ie, $\mathsf{N}^r(S_i)\cong S_{\nu^{-1}(i)}.$
\end{enumerate}
\end{proposition}



\subsection{The Nakayama automorphism}


Let $A$ be a Frobenius algebra with counit $\varepsilon$. Let $\kappa = \langle -, - \rangle : A \otimes A \to k$ be the non-degenerate associative bilinear form defining the Frobenius structure, ie $\langle x,y \rangle = \varepsilon (xy) $. This $\kappa$ is called the \textit{Frobenius form}.

By non-degeneracy, there is a $k$-linear isomorphism
\[
\Psi : A \longrightarrow A^{*}, \qquad \Psi(z) = \langle -, z \rangle.
\]

Now for $a \in A$, we have $\langle a, - \rangle \in A^{*}$. Therefore there is a unique element
\[
a' := \Psi^{-1}\big( \langle a, - \rangle \big) \in A
\]
such that
\[
\langle a, - \rangle = \langle -, a' \rangle \qquad \text{as elements of } A^{*}.
\]

We define
\[
\nu : A \longrightarrow A, \qquad a \longmapsto a'.
\]

By linearity of $\langle -, - \rangle$, this defines a $k$-linear map, and by the associativity of $\langle -, - \rangle$, it is in fact an algebra map.

To see that it is an automorphism, it suffices to check injectivity: if $\nu(a) = 0$, then
\[
\langle a, - \rangle = \langle -, \nu(a) \rangle = 0,
\]
and by non-degeneracy, $a = 0$. This is the \textit{Nakayama automorphism }of $A$. Note that it is actually measuring how far is the Frobenius structure from being symmetric.




What is the relation between the Nakayama automorphism, the Nakayama functor and the Nakayama permutation? It turns out that the Nakayama functor, what is does it to twist the action of $A$ on a given module by the Nakayama automorphism.


More precisely, if $\alpha: A \to A$ is an automorphism of $A$, and $M$ is an $A$-module, let ${}_{\alpha} M$ be $M$ as vector space but with $A$-module structure twisted by $\alpha$,
$$ a * m := \alpha (a) \cdot m. $$


Let $\nu$ be the Nakayama automorphism as before. It turns out that the right Nakayama functor of $A$ twists the $A$-action of the $A$-modules: for any $A$-module $M$ we have
\begin{equation}
\mathsf{N}^r (M) \cong {}_\nu M. 
\end{equation}
so in view of \cref{prop:Naka} we have

$$ {}_\nu S_i \cong \mathsf{N}^r(S_i)\cong S_{\nu^{-1}(i)}.  $$


In other words, for a simple $S$, 

$$ \nu (S) \cong {}_{\nu^{-1}} S.  $$


Same goes for the indecomposable projective / injective objects.

\subsection{Comparison: quasi-Frobenius vs Frobenius algebras}


Any Frob algebra is quasi-Frobenius, but the opposite is NOT true. What is true is that any quasi-Frob algebra is Morita equivalent to a Frob algebra, ie, for a finite category, being quasi-Frobenius = self injective, ie being equivalent to $A-mod$ for a quasi-Frobenius = self injective algebra, is the same as being equivalent to $A-mod$ for a Frobenius algebra.



When is a quasi-Frobenius algebra actually Frobenius?



\begin{theorem}
A fin dim quasi-Frobenius algebra $A$ is Frobenius if and only of $$\dim_\Bbbk \nu(S) = \dim_\Bbbk S$$ for all simple $A$-modules $S$.
\end{theorem}



\subsection{Nakayama functors: Morita-independent version}
...
Let be a finite tensor category with (simple) unit $\mathbf{1}$. Define
\begin{equation}
\alpha:= \mathrm{soc}(P(\mathbf{1})),
\end{equation}
ie the socle of the projective cover of the unit. In other words: if $\nu$ denotes the Nakayama permutation, $$  \alpha = \nu (\mathbf{1}). $$

In particular, this implies that 
$$ \mathsf{N}^r(\alpha) \cong \mathbf{1}.  $$
...
\begin{theorem}[ENO04]
For a finite tensor category, $$ semisimple \Rightarrow unimodular.$$
\end{theorem}
...
We have two different versions of the distinguished invertible object in a finite tensor category. Both coincide:


\begin{proposition}[Shimizu's unimodular Lemma 5.5]
Both definitions of $\alpha$ agree.
\end{proposition}

\begin{proof}
Set $B(X,Y) = \operatorname{Hom}_{\mathcal{C}}(X,\mathbf{1}) \cdot Y$. We denote by $\pi \colon D \xrightarrow{\cdot\cdot} B$ the universal dinatural transformation of the end. For $X_1, X_2 \in \mathcal{C}$, we have
\[
\pi_{X_1 \oplus X_2} = B(\mathrm{id}_{X_1 \oplus X_2}, i_1 p_1) \circ \pi_{X_1 \oplus X_2} + B(\mathrm{id}_{X_1 \oplus X_2}, i_2 p_2) \circ \pi_{X_1 \oplus X_2}
\]
\[
= B(p_1, i_1) \circ \pi_{X_1} + B(p_2, i_2) \circ \pi_{X_2},
\]
where $i_r \colon X_r \to X_1 \oplus X_2$ and $p_r \colon X_1 \oplus X_2 \to X_r$ $(r = 1,2)$ are the inclusion and the projection, respectively. Hence, if $\pi_{X_1} = 0$ and $\pi_{X_2} = 0$, then $\pi_{X_1 \oplus X_2} = 0$.

Now let $V_0 = \mathbf{1}, V_1, \dots, V_n$ be a complete set of representatives of the isomorphism classes of simple objects of $\mathcal{C}$, and let $P_i$ be the projective cover of $V_i$. Suppose that $\pi_{P_i} = 0$ for all $i$. Then, by the above argument, $\pi_P = 0$ for all projective object $P \in \mathcal{C}$. For each $X \in \mathcal{C}$, there are a projective object $P \in \mathcal{C}$ and an epimorphism $f \colon P \to X$. Since $B(f,X)$ is monic and $B(f,\mathrm{id}_X) \circ \pi_X = B(\mathrm{id}_P, f) \circ \pi_P = 0$, we have $\pi_X = 0$. Hence $D = 0$ by the universal property. This contradicts the fact that $D$ is a simple object.

By the above argument, $\pi_{P_i} \neq 0$ for some $i$. Since $B(P_i, P_i) = 0$ for $i \neq 0$, the morphism $\pi_{P_0}$ must be non-zero. Since $D$ is a simple object, the morphism $\pi_0$ induces a monomorphism from $D$ to the socle $S$ of $P_0$. On the other hand, it is known that $S$ is simple [EO]. Thus $D \cong S$.
\end{proof}


\subsection{Self-duality of $\Delta$ in a unimodular ftc}
...
\begin{definition}
A pivotal tensor category $(\mathcal{C}, P : (-) \simeq (-)^{**})$ is \textit{trace spherical} if for all objects $V \in \mathcal{C}$, the quantum trace of the pivotal structure on $V$ agrees with the quantum trace of the pivotal structure on $^*V$, that is $\mathrm{Tr}(P_V) = \mathrm{Tr}(P_{(^*V)})$.
\end{definition}
...
\begin{proof}
Both the property of sphericality and the property of trace sphericality are invariant under base change, so it suffices to compare them over an algebraically closed field. When the base field is algebraically closed, Etingof-Nikshych-Ostrik prove in \textbf{[ENO04]}, Thm 7.3 and Cor 7.4] that the Radford equivalence $\mathcal{R}_{\mathcal{C}} : (-) \xrightarrow{\simeq} (-)^{****}$ is characterized by a property that is equivalent to the following property: for any object $V \in \mathcal{C}$ and any morphism $f_V : V \to V^{**}$, there is an equality of traces $\mathrm{Tr}(f_V) = \mathrm{Tr}((f_V)^* \circ \mathcal{R}_{\mathcal{C}}(^*V))$. Here we have abbreviated $\mathcal{R}_{\mathcal{C}}(^*V)$ as $\mathcal{R}_{(^*V)}$. When $(\mathcal{C}, P)$ is a pivotal tensor category, we have $(P_V)^* = (P_{V^*})^{-1}$ \textbf{[Sel11]}, Lemma 4.11; SR72, Prop 5.2.3]. If $(\mathcal{C}, P)$ is spherical, it follows that
\[
\mathrm{Tr}(P_V)
= \mathrm{Tr}((P_V)^* \circ \mathcal{R}_{(^*V)})
= \mathrm{Tr}(P_{V^*}^{-1} \circ P_{V^*} \circ P_{(^*V)})
= \mathrm{Tr}(P_{(^*V)});
\]
that is, $(\mathcal{C}, P)$ is trace spherical. The converse follows by cyclically permuting this equation.
\end{proof}

\textbf{WARNING:} In the setting of non-semisimple finite tensor categories, it is neither the case that spherical implies trace spherical, nor the case that trace spherical implies spherical.
...
\subsection{The canonical Hopf algebra in a braided finite tensor category}



Let $\C$ be a finite tensor category over a field $k$, braided or otherwise. We consider the bifunctor
\[
T : \C^{\op} \times \C \longrightarrow \C, \qquad (Y, X) \longmapsto X \otimes Y^*.
\]
We wish to form the coend
\[
\F \;=\; \int^{X \in \C} X \otimes X^*.
\]
This is the universal object equipped with a \emph{dinatural transformation}
\[
i_X : X \otimes X^* \longrightarrow \F,
\]
meaning that for every morphism $f : X \to Y$ in $\C$, the following diagram commutes:
\[
\begin{tikzcd}
X \otimes X^* \arrow[r, "f \otimes \id_{X^*}"] \arrow[dr, "i_X"'] & Y \otimes X^* \arrow[d] \\
 & \F \\
X \otimes Y^* \arrow[r, "\id_X \otimes f^*"'] \arrow[ur, "i_Y"] & X \otimes X^* \arrow[u]
\end{tikzcd}
\]
More precisely, $i_Y \circ (\id_Y \otimes f^*) = i_X \circ (f \otimes \id_{X^*})$ for all $f: X \to Y$.

\begin{proposition}
The coend $\F = \int^{X \in \C} X \otimes X^*$ exists in $\C$.
\end{proposition}

\begin{proof}
Since $\C$ is a finite tensor category, it has finitely many isomorphism classes of simple objects $L_1, \ldots, L_n$ and all $\Hom$-spaces are finite-dimensional over $k$. As a $k$-linear abelian category, $\C$ has all finite colimits (it is equivalent, as a $k$-linear category, to the category of finite-dimensional modules over a finite-dimensional $k$-algebra, which is cocomplete over finite diagrams).

Any coend can be computed as a coequalizer. Concretely, $\F$ is the coequalizer of the pair of maps
\[
\bigoplus_{f : X \to Y} Y \otimes X^* \;\;\rightrightarrows\;\; \bigoplus_{Z \in \mathrm{Irr}(\C)} Z \otimes Z^*,
\]
where the two maps send the summand indexed by $f: X \to Y$ to the summands indexed by $X$ and $Y$ via $f \otimes \id_{X^*}$ and $\id_Y \otimes f^*$ respectively, and the coproduct on the right ranges over representatives of isomorphism classes of simple objects (by semisimplicity of the relevant Hom-spaces, one reduces the index set to the finitely many simples and the finitely many morphisms between them). Since both direct sums are \emph{finite}, this coequalizer diagram is finite, and hence exists in $\C$.
\end{proof}



The finiteness of $\C$ (finitely many simples, finite-dimensional hom-spaces) is essential: it reduces the a priori infinitely indexed coproducts in the coend formula to finite direct sums, which exist in any abelian category.




When $\C$ is \textbf{braided}, with braiding $c_{X,Y} : X \otimes Y \xrightarrow{\sim} Y \otimes X$, the object $\F$ acquires the structure of a \textbf{Hopf algebra in $\C$}. All structure maps are defined by specifying a dinatural family into (or out of) $\F$ and invoking the universal property of the coend:


\begin{itemize}
\item \textbf{Multiplication }   $\mu : \F \otimes \F \to \F$ : 


Since $\F \otimes \F \cong \int^{X}\int^{Y} (X \otimes X^*) \otimes (Y \otimes Y^*)$ (the tensor product of coends is the iterated coend, by the Fubini theorem for coends), it suffices to give a dinatural family
\[
\mu_{X,Y} : X \otimes X^* \otimes Y \otimes Y^* \longrightarrow \F.
\]
Define it as the composite
\[
X \otimes X^* \otimes Y \otimes Y^*
\xrightarrow{\id_X \otimes c_{X^*, Y} \otimes \id_{Y^*}}
X \otimes Y \otimes X^* \otimes Y^*
\xrightarrow{\;\sim\;}
(X \otimes Y) \otimes (X \otimes Y)^*
\xrightarrow{\;i_{X \otimes Y}\;}
\F,
\]
where the middle isomorphism uses $(X \otimes Y)^* \cong Y^* \otimes X^*$ and the braiding $c_{X^*,Y}$ is used to permute the middle factors. One verifies dinaturality in both $X$ and $Y$ using the naturality of the braiding and the dinaturality of $i_{(\text{--})}$. The resulting morphism $\mu : \F \otimes \F \to \F$ is associative.


\item \textbf{Unit $\eta : \one \to \F$:}



Set
\[
\eta \;=\; i_{\one} \;\circ\; \lambda^{-1}_{\one} : \one \xrightarrow{\sim} \one \otimes \one^* \xrightarrow{\;i_{\one}\;} \F,
\]
using the canonical isomorphism $\one^* \cong \one$ and the unit constraint $\lambda$.



\item \textbf{Comultiplication $\Delta : \F \to \F \otimes \F$}

Define the dinatural family $\delta_X : X \otimes X^* \to \F \otimes \F$ by
\[
X \otimes X^*
\xrightarrow{\id_X \otimes \coev_Y \otimes \id_{X^*}}
X \otimes Y \otimes Y^* \otimes X^*
\xrightarrow{\id_X \otimes \id_Y \otimes c_{Y^*,X^*}}
X \otimes Y \otimes X^* \otimes Y^*
\xrightarrow{\;i_X \,\otimes\, i_Y\;}
\F \otimes \F,
\]
summing over all $Y$ (or more precisely, inserting a copy via coevaluation $\coev_Y : \one \to Y \otimes Y^*$ for each $Y$ and then using the braiding to sort the dual factors). The braiding $c_{Y^*,X^*}$ rearranges $Y^* \otimes X^*$ into $X^* \otimes Y^*$ so that the two tensor factors land correctly in the two copies of $\F$. The result is coassociative.

\item \textbf{Counit $\varepsilon : \F \to \one$:}

The evaluation morphism $\ev_X : X \otimes X^* \to \one$ is natural (hence dinatural) in $X$. By the universal property of $\F$ it factors uniquely through a morphism
\[
\varepsilon : \F \longrightarrow \one.
\]

\item \textbf{Antipode $S : \F \to \F$:}

In a pivotal braided category, define the dinatural family $s_X : X \otimes X^* \to \F$ by
\[
X \otimes X^*
\xrightarrow{\;c_{X, X^*}\;}
X^* \otimes X
...
\F,
\]
using the braiding to swap $X$ and $X^*$, and the pivotal structure to identify $X \cong X^{**}$. This gives a well-defined map $S : \F \to \F$ satisfying the Hopf antipode axiom $\mu \circ (S \otimes \id) \circ \Delta = \eta \circ \varepsilon = \mu \circ (\id \otimes S) \circ \Delta$.
\end{itemize}

Furthermore, this Hopf algebra comes with a Hopf pairing $$\omega_\C : \F \otimes \F \to \F.$$
As with the other structure maps, it is defined by specifying a dinatural family and invoking the universal property of $\F \otimes \F$ as an iterated coend. Define
\[
\omega_{X,Y} : (X \otimes X^*) \otimes (Y \otimes Y^*) \longrightarrow \one
\]
by the composite
\[
X \otimes X^* \otimes Y \otimes Y^*
\xrightarrow{\;\id_X \otimes c_{X^*,Y} \otimes \id_{Y^*}\;}
X \otimes Y \otimes X^* \otimes Y^*
\xrightarrow{\;\ev_X \otimes \ev_Y\;}
\one \otimes \one
\xrightarrow{\;\sim\;}
\one,
\]
where we use the braiding $c_{X^*,Y}$ to bring $X^*$ next to $X$ and $Y^*$ next to $Y$, and then evaluate both pairs simultaneously. More explicitly, this is the composite
\[
X \otimes X^* \otimes Y \otimes Y^*
\xrightarrow{\;\id \otimes c_{X^*,Y} \otimes \id\;}
X \otimes Y \otimes X^* \otimes Y^*
\xrightarrow{\;c_{X,Y} \otimes c_{X^*,Y^*}\;}
Y \otimes X \otimes Y^* \otimes X^*
\xrightarrow{\;\ev_Y \otimes \ev_X\;}
\one,
\]
where one uses the braiding to arrange evaluation pairings $(X, X^*)$ and $(Y, Y^*)$ next to each other. One checks that this family is dinatural in both $X$ and $Y$, so it descends to a well-defined morphism $\omega : \F \otimes \F \to \one$.




\subsection{Non-degenerate braided finite tensor category}



Let $\C$ be a  braided finite tensor category. We say that $\C$ is \textbf{non-degenerate} if so is $\omega_\C$.



\subsection{Weakly-factorisable braided finite tensor category}

Let $\C$ be a  braided finite tensor category. We say that $\C$ is \textbf{weakly-factorisable} if the map $$ \Omega_\C : \Hom_\C (\one, \F) \to \Hom_\C (\F, \one)  \qquad , \qquad f \mapsto (f \otimes \id_\F)\circ \omega_\C $$
is invertible.



\subsection{Factorisable braided finite tensor category}



Let $\mathcal{C}$ be a braided tensor category with braiding $\sigma$, and let
$\overline{\mathcal{C}}$ denote the finite tensor category $\mathcal{C}^{\mathrm{rev}}$ equipped with the braiding $\overline{\sigma}$ given by
\[
(\overline{\sigma}_{V,W} : V \otimes^{\mathrm{rev}} W \to W \otimes^{\mathrm{rev}} V)
:= (\sigma_{V,W}^{-1} : W \otimes V \to V \otimes W)
\]
for $V, W \in \mathcal{C}$. By using the universal property of the Deligne tensor product, we
define the $k$-linear exact functor $G$ by
\begin{equation}
G : \mathcal{C} \boxtimes \overline{\mathcal{C}} \to \mathcal{Z}(\mathcal{C}), \quad V \boxtimes W \mapsto (V \otimes W, c), \tag{3.10}
\end{equation}
where the natural isomorphism $c$ is given by
\begin{equation}
c_X : V \otimes W \otimes X
\xrightarrow{\mathrm{id}_V \otimes \sigma_{X,W}^{-1}}
V \otimes X \otimes W
\xrightarrow{\sigma_{V,X} \otimes \mathrm{id}_W}
X \otimes V \otimes W \tag{3.11}
\end{equation}
for $X \in \mathcal{C}$. The functor $G$ is in fact a braided tensor functor with the monoidal
structure given by the braiding of $\mathcal{Z}(\mathcal{C})$.

\begin{definition}
The braided finite tensor category $\mathcal{C}$ is \emph{factorizable} if the functor $G$ from above is an equivalence.
\end{definition}




\textbf{IMPORTANT:} Factorisable braided finite tensor category $\Rightarrow$ Unimodular by [EGNO, 8.10.10].


\subsection{Modular category (non-ss)}


The following is Theorem 1.1 in Shimizu's "Non-degeneracy conditions for braided..."


For a braided finite tensor category $\mathcal{C}$, the following assertions are equivalent:
\begin{enumerate}
    \item $\mathcal{C}$ is non-degenerate.
    \item $\mathcal{C}$ is factorizable.
    \item $\mathcal{C}$ is weakly-factorizable.
    \item The M\"{u}ger center of $\mathcal{C}$ is trivial, ie $\mathcal{Z}_2(\C) \simeq \mathsf{Vec}_\Bbbk$.
\end{enumerate}


A ribbon finite tensor category satisfying any of the four equivalent conditions above is called a \textbf{modular tensor category.}



\subsection{Hopf algebra terminology and relation with categorical ones}
...
\begin{enumerate}
\item $H$ is \textbf{quasi-triangular} if....
\item $H$ is \textbf{triangular} if it is quasi-triangular with $R^{-1}=R_{21}$.
\item $H$ is \textbf{factorisable} if it is quasi-triangular and the map $$ H^* \to H \qquad , \qquad \omega \mapsto (\omega \otimes id)(R_{21}R) $$ is an isomorphism (for $H$ not nec fin dim, simply surjective). This is the opposite of being triangular.


Equivalently,  by a result of Schneider this can be characterised by $D(H)\cong (H \otimes H)^{F_R}$ (something is twisted by a cocycle), in particular   $D(H)\cong H \otimes H$ as algebras.

\item $H$ is \textbf{unimodular} if it has left and right integrals and they coincide.

\textbf{Sweedler-Lardson theorem:} $H$ is semisimple if and only if there is a left integral $\Lambda$ with $\varepsilon(\Lambda) \neq 0$.

\textbf{Radford theorem:} Factorisable $\Rightarrow$ Unimodular.


\item $H$ is \textbf{ribbon} if ....
\end{enumerate}



IMPORTANT NOTE: For a fd Hopf algebra, 

-- Semisimple implies unimodular

-- Factorisable implies unimodular.


\begin{proposition} Let $H$ be a fin dim Hopf algebra.
\begin{enumerate}
\item $H$-$\mathsf{mod}$ is a finite tensor category.
\item If $H$ is  quasi-triangular, then $H$-$\mathsf{mod}$ is braided.
\item If $H$ is  triangular, then $H$-$\mathsf{mod}$ is symmetric.
\item If $H$ is factorisable, then $H$-$\mathsf{mod}$ is factorisable. Roughly $$ H\text{-}\mathsf{mod}  \boxtimes H\text{-}\mathsf{mod}  \simeq (H \otimes H)\text{-}\mathsf{mod}  \simeq D(H)\text{-}\mathsf{mod} \simeq \mathcal{Z}(H\text{-}\mathsf{mod}  ).  $$
\item If $H$ is unimodular, then $H\text{-}\mathsf{mod}  $ is unimodular [EGNO, \S 6.5].
\item If $H$ is ribbon, then  $H\text{-}\mathsf{mod}  $ is also ribbon.
\item If $H$ is a factorisable ribbon Hopf algebra, then $H\text{-}\mathsf{mod}  $ is a modular tensor category.
\item If $H$ is a semisimple factorisable ribbon Hopf algebra, then $H\text{-}\mathsf{mod}  $ is a modular fusion category.
\item In particular if $H$ is a semisimple fin dim Hopf algebra over a field of char 0, then $D(H)$ is a semisimple factorizable ribbon Hopf algebra (with balancing element 1) and then $D(H)\text{-}\mathsf{mod}  $ is a modular fusion category.
\end{enumerate}
\end{proposition}
...
\paragraph{Setup.}
Let $A$ and $B$ be finite-dimensional algebras over a field $k$.
Let
\[
M,M' \in A\text{-}\mathrm{Mod}, 
\qquad
N,N' \in \mathrm{Mod}\text{-}B,
\]
where $M,M'$ are left $A$-modules and $N,N'$ are right $B$-modules.

Then $M \otimes_k N$ and $M' \otimes_k N'$ carry natural $A$-$B$-bimodule structures via
\[
a \cdot (m \otimes n) \cdot b := (am) \otimes (nb).
\]

\paragraph{Claim (tensor–Hom adjunction for bimodules).}
There is a canonical isomorphism
\[
\operatorname{Hom}_{A\text{-}B}(M \otimes N,\; M' \otimes N')
\;\cong\;
\operatorname{Hom}_A\!\big(M,\operatorname{Hom}_B(N, M' \otimes N')\big).
\]

\paragraph{Explanation.}
We first apply the usual tensor–Hom adjunction in $\mathrm{Vect}_k$:
\[
\operatorname{Hom}_k(M \otimes N,\; M' \otimes N')
\cong
\operatorname{Hom}_k\!\big(M,\operatorname{Hom}_k(N, M' \otimes N')\big).
\]

We then impose equivariance:

\begin{itemize}
\item $A$-linearity in the first variable replaces $\operatorname{Hom}_k(M,-)$ by $\operatorname{Hom}_A(M,-)$.
\item $B$-linearity in the second variable replaces $\operatorname{Hom}_k(N,-)$ by $\operatorname{Hom}_B(N,-)$.
\end{itemize}

Also 
\[
\operatorname{Hom}_B\!\big(N,\, M' \otimes N'\big)
\]
carries a natural structure of left $A$-module: For $f \in \operatorname{Hom}_B(N, M' \otimes N')$ and $a \in A$, define
\[
(a \cdot f)(n) := a \cdot f(n),
\]
where the action on the right-hand side is the $A$-action on $M' \otimes N'$.
...
\paragraph{Remark.}
There is a symmetric form obtained by currying in the other variable:
\[
\operatorname{Hom}_{A\text{-}B}(M \otimes N,\; M' \otimes N')
\cong
\operatorname{Hom}_B\!\big(N,\operatorname{Hom}_A(M, M' \otimes N')\big).
\]




If $A,B$ are algebras in a finite tensor category $\mathcal{A}$, and $M,M',N,N'$ are $A$/$B$ modules in $\mathcal{A}$, then the formula becomes 
\[
\boxed{
\operatorname{Hom}_{A\text{-}B}(M \otimes N,\; M' \otimes N')
\;\cong\;
\operatorname{Hom}_A\!\big(M,\underline{\operatorname{Hom}}_B(N, M' \otimes N')\big).
}
\]
where $\underline{\operatorname{Hom}}_B(N, M' \otimes N')$ is viewed as a left $A$-module with action map
$$A \otimes\underline{\operatorname{Hom}}_B(N, M' \otimes N') \cong \underline{\operatorname{Hom}}_B(N, A \otimes M' \otimes N') \to \underline{\operatorname{Hom}}_B(N, M' \otimes N').$$
%%%%%%%%%%%%%%%%%%%%%%
%%%%%%%%%%%%%%
...
\end{itemize}
Thanks to the non-degenerate symmetric pairing, we can define a symmetric monoidal functor $$\mathsf{End}_{\mathcal{A}, \kappa}: \mathsf{Graphs} \to \mathsf{Cat}$$ sending a corolla $T$ to the morphism category 
\begin{equation}\label{eq:End_Ak}
\mathsf{End}_{\mathcal{A}, \kappa}(T):=\mathcal{S}(\mathcal{A}^{\boxtimes \mathsf{Legs}(T)}, \mathcal{I})
\end{equation}
from the unordered monoidal product $\mathcal{A}^{\boxtimes \mathsf{Legs}(T)}$ of $\mathcal{A}$ over the set $\mathsf{Legs}(T)$ of legs of $T$ to the monoidal unit $\mathcal{I} \in \mathcal{S}$. This $\mathsf{Cat}$-valued modular operad is the \textit{modular endomorphism operad} of $(\mathcal{A}, \kappa)$ and denoted by $\mathsf{End}_{\mathcal{A}, \kappa}$.
...
\section{Modified traces}


\subsection{Twisted traces}

Let $\mathcal{M}$ be a $\Bbbk$-linear category, let $\mathcal{P}$ be a full subcategory of $\mathcal{M}$,
and let $\Sigma : \mathcal{M} \to \mathcal{M}$ be a $\Bbbk$-linear right exact endofunctor. A
\emph{$\Sigma$-twisted trace on $\mathcal{P}$} is a family
\[
\mathsf{t}_\bullet = \{ \mathsf{t}_P : \mathrm{Hom}_{\mathcal{M}}(P, \Sigma(P)) \to \Bbbk \}_{P \in \mathcal{P}}
\]
of $\Bbbk$-linear maps parametrized by the objects of $\mathcal{P}$ such that the following $\Sigma$-cyclicity
condition is satisfied:

\medskip
\noindent\textbf{$\Sigma$-cyclicity:} For all objects $P, Q \in \mathcal{P}$ and morphisms $f : P \to Q$ and $g : Q \to
\Sigma(P)$ in $\mathcal{M}$, the following equation holds:
\begin{equation}
\mathsf{t}_P\!\left(P \xrightarrow{f} Q \xrightarrow{g} \Sigma(P)\right)
= \mathsf{t}_Q\!\left(Q \xrightarrow{g} \Sigma(P) \xrightarrow{\Sigma(f)} \Sigma(Q)\right). \tag{3.1}
\end{equation}
We say that a $\Sigma$-twisted trace $\mathsf{t}_\bullet$ on $\mathcal{P}$ is \emph{non-degenerate} if the pairing
\[
\mathrm{Hom}_{\mathcal{M}}(M, \Sigma(P)) \times \mathrm{Hom}_{\mathcal{M}}(P, M) \to \Bbbk, \quad (f, g) \mapsto \mathsf{t}_P(fg)
\]
is non-degenerate for all objects $M \in \mathcal{M}$ and $P \in \mathcal{P}$.


Important that $\Sigma$ is not an endofunctor of $\mathcal{P}$ because a major case of interest is $\mathcal{P} = Proj (\mathcal{M})$ and $\Sigma$ the Nakayama functors (that takes proj obj to inj obj).


We write $\mathrm{Tr}(\mathcal{M})$ for the class of $\Sigma$-twisted traces on $Proj(\mathcal{M})$.


\begin{theorem}[SS]
There is a bijection
$$ \mathrm{Nat}(\Sigma, \mathsf{N}^r) \overset{\cong}{\to}  \mathrm{Tr}(\mathcal{M}) $$ via the ``canonical Nakayama twisted trace''. Under this bijection, natural isomorphisms correspond to non-degenerate $\Sigma$-twisted traces.
\end{theorem}



\subsection{Module-compatibility of twisted traces}

Let $\mathcal{C}$ be a rigid monoidal category and let $\mathcal{M}$ be a finite category and a  right $\mathcal{C}$-module category. We write

\[
\mathtt{cl}_{M,N|X} : \mathrm{Hom}_{\mathcal{M}}(M \olessthan X, N \olessthan X^{**}) \to \mathrm{Hom}_{\mathcal{M}}(M, N)
\quad (M, N \in \mathcal{M},\, X \in \mathcal{C}),
\]
\[
\mathtt{cl}_{M,N|X}(f) = (\mathrm{id}_N \olessthan \mathrm{ev}_{X^*}) \circ (f \olessthan \mathrm{id}_{X^*}) \circ (\mathrm{id}_M \olessthan \mathrm{coev}_X).
\]
The linear map $\mathtt{cl}_{M,N|X}$ is called the partial trace in literature (at least when
$\mathcal{M} = \mathcal{C}$ and $\mathcal{C}$ is pivotal), however, we call it the \emph{closing operator} to prevent the proliferation of trace-like terms.


Now let $\Sigma: \mathcal{M} \to \mathcal{M}$ be a $ (-)^{**}$-twisted right exact $\C$-module functor. In other words, $\Sigma$ is a $\Bbbk$-linear right exact endofunctor on $\mathcal{M}$ equipped with a natural
isomorphism
\[
\Psi^{\Sigma}_{M,X} : \Sigma(M) \olessthan X^{**} \to \Sigma(M \olessthan X) \quad (M \in \mathcal{M},\, X \in \mathcal{C})
\]
such that the equations $\Psi^{\Sigma}_{M,\mathbf{1}} = \mathrm{id}_{\Sigma(M)}$ and $\Psi^{\Sigma}_{M, X \otimes Y} = \Psi^{\Sigma}_{M \olessthan X, Y} \circ (\Psi^{\Sigma}_{M,X} \olessthan \mathrm{id}_{Y^{**}})$
hold for all objects $M \in \mathcal{M}$ and $X, Y \in \mathcal{C}$. Now we clarify what `compatible'
means:

\begin{definition}
A $\Sigma$-twisted trace $\mathsf{t}_\bullet$ on $\mathrm{Proj}(\mathcal{M})$ is said to satisfy \textit{the right partial trace property}, or to be \emph{compatible with
the right $\mathcal{C}$-module structure} (or \emph{module-compatible} for short) if the equation
\begin{equation}
\mathsf{t}_P(\mathtt{cl}_{P,\Sigma(P)|X}(f)) = \mathsf{t}_{P \olessthan X}(\Psi^{\Sigma}_{P,X} \circ f) \tag{5.3}
\end{equation}
holds for all $P \in \mathrm{Proj}(\mathcal{M})$, $X \in \mathcal{C}$ and $f \in \mathrm{Hom}_{\mathcal{M}}(P \olessthan X, \Sigma(P) \olessthan X^{**})$.
\end{definition} 
...
\begin{theorem}[SS]
Under the previous bijection
$$ \mathrm{Nat}(\Sigma, \mathsf{N}^r) \overset{\cong}{\to}  \mathrm{Tr}(\mathcal{M}), $$ natural transformations which are module-compatible (ie natural transformations of module functors) correspond to $\Sigma$-twisted traces satisfying the right partial trace property. Natural isomorphisms of module functors correspond to non-degenerate traces.
\end{theorem}
...
Let $\mathcal{M}$ be an \textbf{indecomposable} (ie not equivalent to a direct sum of categories)  exact right $\C$-module category, and let $\Sigma: \mathcal{M} \to \mathcal{M}$ be a $ (-)^{**}$-twisted right exact $\C$-module functor.


Then there exists a non-degenerate module-compatible $\Sigma$-twisted
trace on $\mathrm{Proj}(\mathcal{M})$ if and only if $\Sigma \cong \mathsf{N}_{\mathcal{M}}$ as right $\mathcal{C}$-module functors. If this is
the case, a non-zero module-compatible $\Sigma$-twisted trace on $\mathrm{Proj}(\mathcal{M})$ is unique up to
scalar multiple and every such trace is non-degenerate.



\subsection{Left/right modified traces}


Let $\C$ be a pivotal finite tensor category. The identity of $\C$ becomes a  $(-)^{**}$-twisted left/right  $\C$-module functor thanks to the pivotal structure.

\begin{definition}
A \emph{left} (resp. \emph{right}) \emph{modified trace} on $\mathrm{Proj}(\mathcal{C})$ is an $\mathrm{id}_{\mathcal{C}}$-twisted trace
on $\mathrm{Proj}(\mathcal{C})$ compatible with the left (resp. right) $\mathcal{C}$-module structure. We denote by
$\mathcal{T}_{\mathcal{C}}^{(\ell)}$ and $\mathcal{T}_{\mathcal{C}}^{(r)}$ the spaces of left and right modified traces on $\mathrm{Proj}(\mathcal{C})$, respectively.


In more detail: a left/right modified trace  on on $\mathrm{Proj}(\mathcal{C})$ is  a family
is a family
\[
\mathsf{t}_\bullet = \{ \mathsf{t}_P : \mathrm{End}_{\mathcal{C}}(P) \to \Bbbk \}_{P \in \mathcal{P}}
\]
of $\Bbbk$-linear maps parametrized by the objects of $\mathcal{P}$ such that

\medskip
\noindent\textbf{1) Cyclicity:} For all objects $P, Q \in \mathcal{P}$ and morphisms $f : P \to Q$ and $g : Q \to
P$ in $\mathcal{C}$, the following equation holds: $$\mathsf{t}_Q(f \circ g)  =  \mathsf{t}_P(g \circ f)  $$


\medskip
\noindent\textbf{2)  Left (resp. right) partial trace property:}  If $P\in Proj (\C)$ and $X \in \C$, 

$$ \mathsf{t}_{V \otimes P}(f) = \mathsf{t}_P (\mathtt{cl}_{P|X}(f))  \qquad , \qquad (resp. \ \mathsf{t}_{P \otimes V}(f) = \mathsf{t}_P (\mathtt{cl}_{X|P}(f)) )  $$





\textbf{Key observation:} This closure operator, ie the  left/right categorical aka pivotal trace, is a left/right modified trace. The issue is the following:

\begin{proposition}
Let $\mathcal{C}$ be a pivotal finite tensor category over a field $\Bbbk$. The pivotal trace
$\mathrm{tr} $ is non-zero on
$ \mathrm{Proj}(\mathcal{C})$ if and only if $\mathcal{C}$ is semisimple.
\end{proposition}

\begin{proof}
Recall that in a finite tensor category there is a distinguished projective object
$P_{\mathbf{1}}$, the projective cover of the unit object $\mathbf{1}$. A fundamental theorem of
finite tensor categories states that $\mathcal{C}$ is semisimple if and only if $\mathbf{1}$ is
projective, that is, $P_{\mathbf{1}} \cong \mathbf{1}$.

We first observe that for any projective object $P \in \mathcal{C}$, the pivotal trace of
any endomorphism $f \in \mathrm{End}_{\mathcal{C}}(P)$ factors through $P_{\mathbf{1}}$. Indeed, by
definition of the pivotal trace, $\mathrm{tr}_{P}(f)$ is the composite
\[
\mathbf{1}
\xrightarrow{\mathrm{coev}_{P}}
P \otimes P^{*}
\xrightarrow{f \otimes \mathrm{id}_{P^{*}}}
P \otimes P^{*}
\xrightarrow{\widetilde{\mathrm{ev}}_{P}}
\mathbf{1},
\]
where $\widetilde{\mathrm{ev}}_{P}$ denotes the pivotal evaluation. This composite is a
morphism $\mathbf{1} \to \mathbf{1}$, hence a scalar, and it factors as
\[
\mathbf{1} \xrightarrow{f} P_{\mathbf{1}} \xrightarrow{g} \mathbf{1}
\]
because $P_{\mathbf{1}}$ is the projective cover of $\mathbf{1}$: the morphism
$\mathbf{1} \xrightarrow{\mathrm{coev}_{P}} P \otimes P^{*}$ lands in a projective object
(since $P$ projective implies $P \otimes P^{*}$ projective), and any morphism from a
projective object into $\mathbf{1}$ factors through the projective cover
$P_{\mathbf{1}} \twoheadrightarrow \mathbf{1}$. Thus $\mathrm{tr}_{P}(f)$ factors through
$P_{\mathbf{1}}$ for every projective $P$ and every $f$.

\medskip

$(\Leftarrow)$ Suppose $\mathcal{C}$ is semisimple, so $P_{\mathbf{1}} \cong \mathbf{1}$.
Then the factorisation
\[
\mathbf{1} \xrightarrow{f} P_{\mathbf{1}} \xrightarrow{g} \mathbf{1}
\]
through which $\mathrm{tr}_{P}(\mathrm{id}_{P})$ factors has $P_{\mathbf{1}} = \mathbf{1}$,
so $g \circ f$ is simply a scalar multiple of $\mathrm{id}_{\mathbf{1}}$. In a semisimple
pivotal category every simple object $L$ satisfies
$\dim(L) = \mathrm{tr}(\mathrm{id}_{L}) \neq 0$, since $\mathrm{End}(L) \cong \Bbbk$ by
Schur's lemma and the Hom-pairing is non-degenerate. As the trace is additive on
direct sums, it follows that $\mathrm{tr}_{P} \not\equiv 0$ for every non-zero
projective $P$.

\medskip

$(\Rightarrow)$ We prove the contrapositive. Suppose $\mathcal{C}$ is not semisimple,
so $P_{\mathbf{1}} \ncong \mathbf{1}$, meaning the projective cover map
$\pi : P_{\mathbf{1}} \twoheadrightarrow \mathbf{1}$ does not split. We claim that for
every projective object $P$, the trace $\mathrm{tr}_{P}(\mathrm{id}_{P}) = 0$, that is,
the composite
\[
\mathbf{1}
\xrightarrow{\mathrm{coev}_{P}}
...
\mathbf{1}
\]
is zero. By the factorisation observation above, this composite factors as
\[
\mathbf{1} \xrightarrow{f} P_{\mathbf{1}} \xrightarrow{g} \mathbf{1}
\]
for some morphisms $f$ and $g$. Since $\mathcal{C}$ is not semisimple, the sequence
\[
0
\to
\mathrm{rad}(P_{\mathbf{1}})
\to
P_{\mathbf{1}}
\xrightarrow{\pi}
\mathbf{1}
\to 0
\]
is a non-split exact sequence with $\mathrm{rad}(P_{\mathbf{1}}) \neq 0$. Any morphism
$g : P_{\mathbf{1}} \to \mathbf{1}$ is a scalar multiple of $\pi$, and any morphism
$f : \mathbf{1} \to P_{\mathbf{1}}$ would split $\pi$ if $g \circ f \neq 0$. Since $\pi$
does not split, there is no morphism $f : \mathbf{1} \to P_{\mathbf{1}}$ with
$g \circ f \neq 0$, and therefore $g \circ f = 0$ for all such $f$ and $g$. Hence
\[
\mathrm{tr}_{P}(\mathrm{id}_{P}) = g \circ f = 0
\]
for every projective object $P$, and by linearity
$\mathrm{tr}_{P} \equiv 0$ on all of $\mathrm{End}_{\mathcal{C}}(P)$.
\end{proof}




\textbf{Question:} when does a pivotal finite tensor category admit modified traces on $Proj (\C)$? 




\begin{theorem}
For a pivotal finite tensor category $\mathcal{C}$, the following assertions are
equivalent:
\begin{enumerate}
    \item $\mathcal{C}$ is unimodular, that is, there is an isomorphism $\alpha_{\mathcal{C}} \cong \mathbf{1}$ in $\mathcal{C}$.
    \item There is a non-zero right modified trace on $\mathrm{Proj}(\mathcal{C})$.
    \item There is a non-zero left modified trace on $\mathrm{Proj}(\mathcal{C})$.
\end{enumerate}
If these equivalent conditions are satisfied, then we have $\dim_{\Bbbk} \mathcal{T}_{\mathcal{C}}^{(r)} = \dim_{\Bbbk} \mathcal{T}_{\mathcal{C}}^{(\ell)} = 1$
and every non-zero left or right modified trace on $\mathrm{Proj}(\mathcal{C})$ is non-degenerate.
\end{theorem}


\subsection{Two-sided modified traces}


Let $\mathcal{C}$ be a pivotal finite tensor category. A \emph{two-sided modified trace}
on $\mathrm{Proj}(\mathcal{C})$ is an element of $\mathcal{T}_{\mathcal{C}}^{(\ell r)} :=
\mathcal{T}_{\mathcal{C}}^{(\ell)} \cap \mathcal{T}_{\mathcal{C}}^{(r)}$.


Recall that a pivotal finite tensor category is \textit{spherical} (according to Douglas-Schommer-Pries-Snyder) if it is unimodular, ie $\alpha \cong \mathbf{1}$, and the trivialisation $(-)^{\vee \vee \vee \vee} \cong \mathrm{Id}$ coming from the Radford isomorphism coincides with the   trivialisation $(-)^{\vee \vee \vee \vee} \cong (-)^{ \vee \vee} \cong  \mathrm{Id}$ induced by the pivotal structure.



\begin{corollary}
Let $\mathcal{C}$ be a pivotal finite tensor category. Then 
$$ \exists
\textrm{
 non-zero
two-sided modified trace on } \mathrm{Proj}(\mathcal{C}) \iff \mathcal{C} \text{
spherical} $$ If this is the case, a non-zero two-sided modified trace on
$\mathrm{Proj}(\mathcal{C})$ is unique up to scalar multiple and every such trace is
non-degenerate.
\end{corollary}
...
If $\C$ is ribbon, the situation is even more interesting:


\begin{corollary}
Let $\C$ be a ribbon finite tensor category. Then TFAE:
\begin{enumerate}
\item $\C$ is spherical,
\item $\C$ is unimodular,
\item There exists a non-zero two-sided modified trace on $Proj (\C)$ (in this case such a mod trace is always non-degenerate by the corollary above).
\end{enumerate}
\end{corollary}
...
\begin{corollary}
Any modular tensor categories admits a non-deg two-sided modified trace.
\end{corollary}

Reason? Factorisable braided finite tensor category $\Rightarrow$ Unimodular by [EGNO, 8.10.10], and the corollary above.
...
\subsection{Bakalov-Kirillov's modular functors}


\begin{definition}[Naïve version]
Let $\mathsf{Mnfd}_d$ be the category with objects homeomorphism classes of $d$-manifolds and arrows only endomorphisms given by $\mathrm{End}(M)=MCG(M).$ This is a symmetric monoidal category with respect to disjoint union.

A $d$-dimensional modular functor is a strong monoidal functor $$\tau: \mathsf{Mnfd}_d \to \mathsf{vect}_\Bbbk$$ with the normalisation property that $\tau (S^d)=\Bbbk$.
\end{definition}



Ie this is a system of MCG representations. 

Now we put $d=2$.  Ideally one would like to glue surfaces and say that the MCG representations are coherent with respect to glueing. For this we need \textit{extended surfaces}, ie surfaces with parametrised boundary (=circle) components. Homeomorphisms between extended surfaces are supposed to preserve the boundary parametrisations.




\begin{definition}
Let $\mathcal{C}$ be an abelian category over a field $k$, and let $R=\bigoplus V_i \boxtimes V_i^* $ be a symmetric object in $\mathrm{ind}\text{-}\mathcal{C}^{\boxtimes 2}$ (see Section~2.4). Then a \emph{$\mathcal{C}$-extended modular functor} is the following collection of data:

\begin{enumerate}
\item[(i)] To every extended surface $\Sigma$ is assigned a polylinear functor
\[
\tau(\Sigma) \colon \mathcal{C}^{\boxtimes \pi_0(\partial \Sigma)} \longrightarrow \mathsf{vect}_\Bbbk,
\]
where $\pi_0(\partial \Sigma)$ is the set of boundary components (or punctures, depending on the point of view) of $\Sigma$. In other words, for every choice of objects $W_a \in \mathcal{C}$ attached to every boundary component of $\Sigma$ (so, $a$ runs through the set of connected components of $\partial \Sigma$) is assigned a finite-dimensional vector space $\tau(\Sigma; \{W_a\})$, and this assignment is functorial in $W_a$.

\item[(ii)] To every homeomorphism $f \colon \Sigma \xrightarrow{\sim} \Sigma'$ is assigned a functorial isomorphism $f_* \colon \tau(\Sigma) \xrightarrow{\sim} \tau(\Sigma')$, which depends only on the isotopy class of $f$.

\item[(iii)] Functorial isomorphisms $\tau(\emptyset) \xrightarrow{\sim} k$, $\tau(N_1 \sqcup N_2) \xrightarrow{\sim} \tau(N_1) \otimes \tau(N_2)$.

\item[(iv)] \textbf{Gluing isomorphism:} Let $(\partial \Sigma)_\alpha$, $(\partial \Sigma)_\beta$ be two different boundary components of $\Sigma$, and let $\pi_\alpha \colon (\partial \Sigma)_\alpha \xrightarrow{\sim} S^1$, $\pi_\beta$ be their parameterizations. Define a new surface $\sqcup_{\alpha,\beta} \Sigma$ by identifying all points $p \in (\partial \Sigma)_\alpha$ with $\pi_\beta^{-1}(-\overline{\pi_\alpha(p)}) \in (\partial \Sigma)_\beta$. Let us write $R = \bigoplus_i A_i \boxtimes B_i$. Then we have a functorial isomorphism
\begin{equation}
G_{\alpha,\beta} \colon \bigoplus_i \tau(\Sigma; \{W_a\}, A_i, B_i) \xrightarrow{\sim} \tau(\sqcup_{\alpha,\beta}\Sigma; \{W_a\}), \tag{5.1.1}
\end{equation}
where $A_i$, $B_i$ correspond to the boundary components $(\partial \Sigma)_\alpha$, $(\partial \Sigma)_\beta$, respectively. In the notation of Section~2.4, this can be written as:
\begin{equation}
G_{\alpha,\beta} \colon \tau(\Sigma; \{W_a\}, R^{(1)}, R^{(2)}) \xrightarrow{\sim} \tau(\sqcup_{\alpha,\beta}\Sigma; \{W_a\}). \tag{5.1.2}
\end{equation}
\end{enumerate}
...
\medskip
\noindent \textbf{Multiplicativity:} $(fg)_* = f_* g_*$, $\mathrm{id}_* = \mathrm{id}$.

\smallskip
\noindent \textbf{Functoriality:} All isomorphisms in parts (iii), (iv) above are functorial in $\Sigma$.


\noindent \textbf{Compatibility:} All isomorphisms in parts (iii), (iv) above are compatible with each other.

\smallskip
\noindent \textbf{Symmetry of gluing:} After the identification $R \simeq R^{\mathrm{op}}$, we have $G_{\alpha,\beta} = G_{\beta,\alpha}$.

\smallskip
\noindent \textbf{Normalization:} $\tau(S^2) = k$.

\bigskip

As before, we leave it to the reader to write the explicit statements of all functoriality and compatibility axioms, taking as an example the definitions in Section~4.2. Let us give two of them.

\medskip
\noindent \textbf{Compatibility of gluing with disjoint union:} Let $\Sigma'$, $\Sigma''$ be two extended surfaces, and $(\partial \Sigma')_\alpha$, $(\partial \Sigma')_\beta$ be two different boundary components of $\Sigma'$. Then the following diagram is commutative:
\[
\begin{tikzcd}[column sep=huge, row sep=large]
\tau(\Sigma' \sqcup \Sigma''; \{W_a'\}, R^{(1)}, R^{(2)}, \{W_b''\}) \arrow[r, "G_{\alpha,\beta}"] \arrow[d]
& \tau(\Sigma; \{W_a'\}, \{W_b''\}) \arrow[d] \\
\tau(\Sigma'; \{W_a'\}, R^{(1)}, R^{(2)}) \otimes \tau(\Sigma''; \{W_b''\}) \arrow[r, "G_{\alpha,\beta} \otimes \mathrm{id}"]
& \tau(\sqcup_{\alpha,\beta}\Sigma'; \{W_a'\}) \otimes \tau(\Sigma''; \{W_b''\})
\end{tikzcd}
\]
where the vertical arrows are the isomorphisms from part (iii), and $\Sigma = \sqcup_{\alpha,\beta}(\Sigma' \sqcup \Sigma'') \simeq (\sqcup_{\alpha,\beta}\Sigma') \sqcup \Sigma''$ (with the homeomorphism isotopic to the identity).


\noindent \textbf{Associativity of gluing:} If $(\partial \Sigma)_\alpha, \dots, (\partial \Sigma)_\delta$ are four different boundary components of $\Sigma$, then:
\begin{equation}
G_{\alpha,\beta} G_{\gamma,\delta} = G_{\gamma,\delta} G_{\alpha,\beta}. \tag{5.1.3}
\end{equation}

In more detail, the following diagram is commutative:
\[
\begin{tikzcd}[column sep=huge, row sep=large]
\tau(\Sigma; \{W_a\}, R'^{(1)}, R'^{(2)}, R''^{(1)}, R''^{(2)}) \arrow[r, "G_{\alpha,\beta}"] \arrow[d, "G_{\gamma,\delta}"']
& \tau(\sqcup_{\alpha,\beta}\Sigma; \{W_a\}, R''^{(1)}, R''^{(2)}) \arrow[d, "G_{\gamma,\delta}"] \\
\tau(\sqcup_{\gamma,\delta}\Sigma; \{W_a\}, R'^{(1)}, R'^{(2)}) \arrow[r, "G_{\alpha,\beta}"']
& \tau(\Sigma'; \{W_a\})
\end{tikzcd}
\]
where $R'$, $R''$ are two copies of $R$, and $\Sigma' = \sqcup_{\alpha,\beta} \sqcup_{\gamma,\delta} \Sigma \simeq \sqcup_{\gamma,\delta} \sqcup_{\alpha,\beta} \Sigma$ (with the homeomorphism isotopic to the identity).

\paragraph{Remark}
Let us give a modification of Definition~5.1.13 which will be useful later. Instead of extended surfaces $\Sigma$, consider pairs $(\Sigma, \varphi)$ where $\Sigma$ is an extended surface and $\varphi \colon \pi_0(\partial \Sigma) \xrightarrow{\sim} S$ is a labeling of the boundary components of $\Sigma$ with a finite set $S$. Then the functor $\tau(\Sigma) \colon \mathcal{C}^{\boxtimes \pi_0(\partial \Sigma)} \to \mathcal{V}ec_f$ from Definition~5.1.13(i) gives rise to a functor $\tau(\Sigma, \varphi) \colon \mathcal{C}^{\boxtimes S} \to \mathcal{V}ec_f$; explicitly, $\tau(\Sigma, \varphi) = \tau(\Sigma) \circ \varphi_*^{-1}$, where $\varphi_* \colon \mathcal{C}^{\boxtimes \pi_0(\partial \Sigma)} \to \mathcal{C}^{\boxtimes S}$ is the functor induced from $\varphi$. It is clear that for any bijection $\psi \colon S \xrightarrow{\sim} S'$, we have $\tau(\Sigma, \psi\varphi) = \tau(\Sigma, \varphi) \circ \psi_*^{-1}$. So a modular functor can be defined as a collection of functors $\tau(\Sigma, \varphi)$ satisfying obvious equivariance conditions.





\subsection{Open  modular functors}



This is a simpler version of modular functor, where we do not glue along boundary components but along parametrised marked intervals.




 The \emph{open surface operad} $\mathsf{O}$~\cite{giansiracusa,sn,envas} is the groupoid-valued operad whose groupoid $\mathsf{O}(n)$ of operations of arity $n$ has as
 objects compact, oriented surfaces with at least one boundary component per connected component together with $n+1$ parametrized intervals embedded in their boundary; the morphisms are mapping classes
 (isotopy classes of diffeomorphisms preserving the orientation and the parametrized intervals),
  and operadic composition is given by gluing surfaces along the parametrized intervals. The open surface operad is in fact a \emph{modular operad} in the sense of \cite{gkmod}.

\begin{definition}
An \emph{open modular functor}, also referred to as a \emph{two-dimensional categorified open topological field theory},
  is defined as a  modular $\mathsf{O}$-algebra that (at least in the context of this article) takes values  in the symmetric monoidal bicategory $\Lexf$ of finite categories, left exact functors and natural transformations.
\end{definition}
  Such a structure is exactly the mathematical description
  of the monodromy data of an \emph{\underline{open} conformal field theory}; this is informed by the notions
  \cite{Lazaroiu,costellotcft,ffrsunique} 
   and defined in this form \cite[Section~8]{microcosm} in the context of the modular microcosm principle.


Let us unpack the definition: a $\Lexf$-valued
  	 open modular functor $\mathfrak{B}$ consists of the data of a finite category $\cA$ (ie the object underlying the  algebra itself) and a rule that associates to every surface $\Sigma$ with at least one boundary component per connected component with $n$ marked boundary intervals  a left exact functor (recall \eqref{eq:End_Ak})
  	 $$ \mathfrak{B}(\Sigma;-) : \cA^{\boxtimes n} \to \mathsf{vect},  $$  	 
ie given $n$ objects $X_1, \ldots, X_n \in \cA$ attached to the boundary intervals, we get a vector space $\mathfrak{B}(\Sigma;X_1, \ldots, X_n)$, the so-called \emph{space of (open) conformal blocks}, which carries an action of the mapping class group of the surface and is compatible with gluing along parametrized intervals. By definition, the space of conformal blocks is left exact in the boundary labels.

$$\includegraphics[width=1\textwidth]{open_mod_functor.png}$$


Some boundary components may have no
parametrized boundary intervals; they are called \textit{free boundary circles}.



According to \cite[Theorem 2.2]{envas},  a $\Lexf$-valued open modular functor is, by evaluation on disks with marked intervals in their boundary, equivalent to the datum of a \emph{pivotal Grothendieck-Verdier category} in $\Lexf$ in the sense of \cite{bd}. In particular, the underlying pivotal finite tensor category of a finite ribbon category $\cA$ extends to a unique open modular functor that we denote by $\Ao$. 
